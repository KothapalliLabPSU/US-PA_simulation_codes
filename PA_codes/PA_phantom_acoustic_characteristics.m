...
% OUTPUT: density and sound speed maps, to be used in photoacoustic back-propagation simulation code next

clc; clear all; close all;
%% Loading the phantom 
phantom = rgb2gray(imread('phantom_image.png')); % include phantom image here
[rows, cols] = size(phantom);

figure; imshow(phantom,[]);

phantom_map = phantom;
if (mod(rows, 2) == 0) && (mod(cols, 2) == 0) %adjusting the phantom if not dimensioned with odd numbers
...
sound_speed_mat = zeros(size(phantom_map)); % m/s
density_mat = zeros(size(phantom_map)); % kg/m^3
...
%
% contrast levels can be referenced from the contrast dictionary in the instruction
% manual
%
    ...
    for j = 1:cols    
        switch phantom_map(i,j)
            ...
            case 150 % bladder
                ...
                density = 1020;
                density_mat(i,j) = randsample(class_2,1) * density; % 0.2% density variation
             
            case 255 % blood targets (HbO)
                sound_speed_mat(i,j) = 1540;
                density = 1045; 
                density_mat(i,j) = randsample(class_0,1) * density; % 0.05% density variation
        end
    end
end
