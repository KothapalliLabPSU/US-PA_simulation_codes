...
map_x(1,:) = -(cols/20):0.1:(cols/20); % adjust depending on pixel:node ratio (currently .1)
map_y(1,:) = 0:0.1:(rows/10); % see above comment
map_y = fliplr(map_y);
...
phantom_size = [rows, cols]; % size if dimensions are odd
save('phantom_size.mat', phantom_size);
...
if (mod(rows, 2) == 0) && (mod(cols, 2) == 0) %adjusting the phantom if not properly dimensioned
    ...
    phantom_size = [rows+1, cols+1]; % adjusted size
    save('phantom_size.mat', phantom_size);
end
...
for i = 1:length(mesh_spec.nodes)
    ...
    map_val = phantom_map(row_ind,col_ind);
